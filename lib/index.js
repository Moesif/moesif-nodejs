...
module.exports = function (options) {
  ...
  moesifMiddleware.startCaptureOutgoing = function() {
    if (moesifMiddleware._mo_patch) {
      logMessage(options.debug, 'startCaptureOutgoing', 'already started capturing outgoing requests.');
    } else {
      function patchLogger(text) {
        logMessage(options.debug, 'outgoing capture', text);
      }
      var recorder = createRecorder(moesifController, options, patchLogger);
...
function logEvent(chunk, req, res, options, moesifController) {
  ...
  if (options.samplingPercentage) {
    ...
    if (randomNumber > options.samplingPercentage) {
      logMessage(options.debug, 'logEvent', 'skipped due to sampling percentage: ' + options.samplingPercentage + ' and random number: ' + randomNumber);
      return;
    }
  }
