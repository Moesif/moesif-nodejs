        ...
        if (!req.body && getRawBodyPromise) {
          ...
          getRawBodyPromise
            ...
            .then(() => {
              ...
              logEventOnResponseEnd(finalBuf, req, res, options, trySaveEventLocal);
...
function logEventOnResponseEnd(chunk, req, res, options, saveEvent) {
  logMessage(options.debug, 'logEventOnResponseEnd', 'reqUrl=' + req.originalUrl);
  logMessage(options.debug, 'logEventOnResponseEnd', 'chunk=', chunk);
  // res.time = new Date();
  // res.responseTime = new Date() - req._startTime;
  ...
  if (options.logBody) {
    ...
    if (req._moBody) {
      ...
      logMessage(options.debug, 'logEventOnResponseEnd', 'processing req._moBody');
      ...
      logMessage(options.debug, 'logEventOnResponseEnd', 'processing req.body');
