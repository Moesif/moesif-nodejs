...
function appendChunk(buf, chunk) {
  ...
  if (chunk) {
    if (Buffer.isBuffer(chunk)) {
      resultBuf = buf ? Buffer.concat([buf, chunk]) : Buffer.from(chunk);
    } else if (typeof chunk === 'string') {
      resultBuf = buf ? Buffer.concat([buf, Buffer.from(chunk)]) : Buffer.from(chunk);
    } else if (typeof chunk.toString === 'function') {
      resultBuf = buf ? Buffer.concat([buf, Buffer.from(chunk.toString())]) : Buffer.from(chunk.toString());
    } else {
