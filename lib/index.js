...
module.exports = function(options) {
  ...
  var saveEventLocal = function(eventData) {
    if (batcher) {
  ...
  var moesifMiddleware = function(req, res, next) {
    ...
    if (
      ...
      try {
        logMessage(options.debug, 'moesifMiddleware', 'request have content but request body is not set, using getRawBody');
        getRawBodyPromise = getRawBody(req, {
          encoding: contentType.parse(req).parameters.charset
        });
      } catch (err) {
        logMessage(options.debug, 'getRawBody', 'getRawBody error' + err);
      }
      // function(err, string) {
      //   logMessage(
      //     options.debug,
      //     'getRawBody',
      //     'callback start error=' + err + ' string=' + string
      //   );
    ...
    res.end = function(chunk, encoding, callback) {
      ...
      try {
        if (getRawBodyPromise) {
          getRawBodyPromise
            .then((str) => {
              logMessage(options.debug, 'getRawBodyPromise', 'successful. append request object with raw body');
            ...
            .then(() => {
              logEvent(finalBuf, req, res, options, saveEventLocal);
            })
            .catch((err) => {
  ...
  moesifMiddleware.startCaptureOutgoing = function() {
    if (moesifMiddleware._mo_patch) {
      ...
      var recorder = createRecorder(saveEventLocal, options, patchLogger);
...
function logEvent(chunk, req, res, options, saveEvent) {
  if (options.samplingPercentage) {
    ...
    // randomly number between 1 and 100.
    if (randomNumber > options.samplingPercentage) {
      logMessage(
        options.debug,
        'logEvent',
        'skipped due to sampling percentage: ' +
...
function ensureValidOptions(options) {
  ...
  if (options.samplingPercentage && !_.isNumber(options.samplingPercentage)) {
    throw new Error('samplingPercentage should be a number');
  }
