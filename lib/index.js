...
module.exports = function(options) {
  ...
  var trySaveEventLocal = function(eventData) {
    ...
    if (moesifConfigManager.shouldSend()) {
      if (batcher) {
...
function applyTxId(event) {
  if (event && event.request && event.request.headers && event.response && event.response.headers) {
    const txId = event.request.headers[TRANSACTION_ID_HEADER] || uuid4();

    event.request.headers[TRANSACTION_ID_HEADER] = txId;
    event.response.headers[TRANSACTION_ID_HEADER] = txId;
  }
}
