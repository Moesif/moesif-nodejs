...
function hashSensitive(jsonBody, debug) {
    ...
    return jsonBody.map(function (item) {
      ...
      if (itemType === 'number' || itemType === 'string') {
        ...
        if (creditCardCheck.isValid) {
          logMessage(debug, 'hashSensitive', 'looks like a credit chard, performing hash.');
          return hash(item).toString();
        }
      }

      return hashSensitive(item, debug);
    });
  }
  ...
  if (typeof jsonBody === 'object') {
    ...
    Object.keys(jsonBody).forEach(function (key) {
      ...
      if (key.toLowerCase() === 'password' && typeof innerVal === 'string') {
        ...
        if (creditCardCheck.isValid) {
          logMessage(debug, 'hashSensitive', 'a field looks like credit chard, performing hash.');
          returnObject[key] = hash(jsonBody[key]).toString();
        } else {
          returnObject[key] = hashSensitive(innerVal, debug);
        }
      } else {
