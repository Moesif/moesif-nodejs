  ...
  var moesifMiddleware = function (req, res, next) {
    ...
    if (!req.body &&
      ...
      getRawBody(req, {
        ...
        if (!err) {
          if (isJsonHeader(req.headers) || startWithJson(string)) {
            req._jsonbody = safeJSONParse(string);
          } else {
            req._moesif_transfer_encoding = 'base64';
            req._moesif_base64_body = bodyToBase64(string);
          }
        }
      })
    }
...
function logEvent(chunk, req, res, options, moesifController) {
  ...
  if(req._moesif_transfer_encoding === 'base64') {
    logData.request.transfer_encoding = 'base64';
    logData.request.body = req._moesif_base64_body;
  } else if (req._jsonbody) {
    logData.request.body = req._jsonbody;
  } else if (req.body) {
    // console.log('there is a request body and type is ' + typeof req.body);
    // console.log(req.body);

    var isReqBodyJson = isJsonHeader(req.headers) || startWithJson(req.body);

    if (typeof req.body === 'object') {
      logData.request.body = req.body;
    } else if (isReqBodyJson) {
      logData.request.body = safeJSONParse(req.body);
    } else {
      logData.request.transfer_encoding = 'base64';
  ...
  // console.log('about to process chunk');

  if (chunk) {
    // var isJson = (res._headers && res._headers['content-type']
    // && res._headers['content-type'].indexOf('json') >= 0);

    var isJson = isJsonHeader(res._headers) || startWithJson(chunk) || typeof chunk === 'object';
  ...
  if (!options.skip(req, res)) {
    ...
    moesifController.createEvent(new EventModel(logData), function(err) {
      ...
      if (err) {
        ...
        // console.log('moesifapi callback done');
        if(options.callback) {
  ...
  try {
    ...
    return {
      moesif_error: {
        ...
        src: 'moesif-express',
        msg: 'Body is not a valid JSON Object or JSON Array',
        args: str
      }
    };
  }
