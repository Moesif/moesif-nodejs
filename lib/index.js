    ...
    batcher = createBatcher(
      function(eventArray) {
        moesifController.createEventsBatch(
          ...
          function(err) {
            if (err) {
              logMessage(options.debug, 'saveEventsBatch', 'moesif API failed with error: ' + JSON.stringify(err));
              if (options.callback) {
                options.callback(err, eventArray);
              }
            } else {
              logMessage(options.debug, 'saveEventsBatch', 'moesif API succeeded with batchSize ' + eventArray.length);
              if (options.callback) {
