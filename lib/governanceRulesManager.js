...
GovernanceRulesManager.prototype.tryGetRules = function () {
  ...
  return new Promise(function (resolve, reject) {
    if (!that._loading && that.shouldFetch()) {
      ...
      moesifController.getRules(function (err, response, event) {
        ...
        if (response && response.statusCode === 200) {
          ...
          try {
            that._rules = response.body;
            that._cacheRules(that._rules);
...
GovernanceRulesManager.prototype._getApplicableUserRules = function (
  ...
  console.log('userRulesHashByRuleId' + JSON.stringify(userRulesHashByRuleId, null, '  '));
  ...
  console.log('here userRulesHashByRuleId ' + JSON.stringify(userRulesHashByRuleId, null, '  '));
  console.log('object values' + Object.values(userRulesHashByRuleId));
...
GovernanceRulesManager.prototype.applyRuleList = function (applicableRules, responseHolder, configRuleValues) {
